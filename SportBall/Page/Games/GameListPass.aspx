...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script id="footTemplate" type="text/x-jquery-tmpl">
     ...
     <tr {{if $data[13] == 0}}class="graybg"{{/if}}>
            ...
            <td {{if $data[46] == 1}}class="graybg"{{/if}}>
                {{html GetDY(this)}}
            </td>
             <td {{if $data[27] == 1}}class="graybg"{{/if}}>
                {{html GetRF(this)}}
            </td>
             <td {{if $data[38] == 1}}class="graybg"{{/if}}>
                {{html GetDX(this)}}
            </td>
             <td {{if $data[54] == 1}}class="graybg"{{/if}}>
                {{html GetDS(this)}}
            </td>
        </tr>
     </script>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder11" runat="server">
    ...
    <ul class="breadcrumb" style="margin-top: 5px">
        当前位置： <a href="#"><span id="lblballType"></span>操盘</a> / <span id="lblball"></span>过关
    </ul>
    <div class="title_right">
        <strong>过关</strong>
    </div>
    <input id="hidPlayType" name="hidPlayType" type="hidden" value="3" />
