// Ver: 4.2.30519
var mIntParentWidth = 0; //上一層項目寬度
var mBolLevel3 = false;//判斷是否為第三層及以上
...
if(nDM&&!nNN4)
{
    ...
    s += ".menuBar{}\n"; //菜單項樣式-無底色
    s += ".menuBarOver{}\n"; //菜單項選中樣式
    s += ".menuItem{background:#58595A;color:#FFFFFF;}\n"; //菜單子項樣式-深色
    s += ".menuItemOver{background:#FAAF40;color:#FFFFFF;}\n"; //菜單子項選中樣式
    s += ".menuLine{background:#EE4036;}\n"; //菜單邊框線條
    s += ".st_tbcss.st_divcss,.st_ftcss{border:1px solid #585858;padding:0px;margin:0px;}\n";
    s += "</STYLE>";
	for(var j=0;j<10;++j)
...
function stittx(i) {
	...
	else
	{
		...
		s+=">";
		if(ityp==6)
		    s += "<TD class=st_tdcss NOWRAP VALIGN=MIDDLE ID=" + ids + "MTD HEIGHT=" + (p.pver ? ssiz : "100%") + " WIDTH=" + (p.pver ? "100%" : ssiz) + ">" + stgimg(simg, ids + "LINE", simw, simh, 0) + "</TD>";
		else
		{
			if(p.pver&&p.plmw||!p.pver&&iicw)
                ...
                s += "<TD class=st_tdcss NOWRAP HEIGHT=" + intHeight + "px STYLE='color:" + itxc[0] + ";padding:" + p.ppad + "px;' ID=" + ids + "MTD ALIGN=" + ihal + " VALIGN=MIDDLE>";
			...
			s+="</TD>";
			if(p.pver&&p.prmw||!p.pver&&iarw)
			    s += "<TD class=st_tdcss NOWRAP ALIGN=CENTER VALIGN=MIDDLE HEIGHT=" + intHeight + "px STYLE='padding:" + p.ppad + "px' ID=" + ids + "RTD" + stgaws(i) + ">" + stgimg(iarr[0], ids + "ARROW", iarw, iarh, iarb) + "</TD>";
		}
		s+="</TABLE>";
		if(!nOP&&!nIE)
			s+="</DIV>";
...
function stpcss(p) {
    var str = "";
    with (p) {
        var str = "border-style:" + pbds + ";border-width:" + ipbw + "px;border-color:" + pbdc + ";";
        str += (nIE ? "background:" + pbgd + ";" : "background-color:" + pbgc + ";" + (pbgi ? "background-image:url(" + pbgi + ");background-repeat:" + pbgr + ";" : ""));
    }
    return str;
}
function stfcss(p)
...
function sticss(i,n) {
    var str = "";
    with (i) {
        var str1 = "border-style:" + ibds + ";border-width:" + ipbw + "px;border-color:" + ibdc[n] + ";";
        str1 += !nIEM && ibgi[n] ? "background-image:url(" + ibgi[n] + ");background-repeat:" + ibgr[n] + ";" : "";
        str = ityp != 6 ? str1 : "";
        str += nIEM && ityp != 6 ? "background:" + ibgd[n] + ";" : "background-color:" + ibgc[n] + ";";
        str += "cursor:" + (nIEM ? "default" : stgcur(i)) + ";";
    }
    return str;
}
function sttcss(i,n)
...
function funMenuOut(objItem)
{
    objItem.style.background = "";
}
//主菜單選擇後的背景色
function funMenuOver(objItem)
{
    objItem.style.background = "#F1F1F1";
}
function stitov(e, o, i) {
    //alert(o.id);
    mIntParentWidth = o.offsetWidth;
    if (document.getElementById(o.id).innerHTML.indexOf("▶")>0)
        mBolLevel3 = true ; //是第三層及以上
    else
        mBolLevel3 = false; //不是第三層及以上
	var p=stgpar(i);
	if(nIEW)
	{
		if(!i.layer)
...
function stppou(e,o,p)
{
    ...
    m.hdid = w.setTimeout("sthdall(st_ms['" + m.mei + "'],0);", 1);
}
function stshst(i,n)
{
	with(i)
	...
	if(xy&&(p.ppi||stgme(p).mtyp))
	{
		...
		else if(nOP)
		{
		    ...
		    if (mBolLevel3 == false) {
		        s.left = xy[0] + "px";
		        s.top = xy[1] + "px";
		        //alert(l.offsetWidth + ":" + l.clientWidth + ":" + mIntParentWidth);
		        if (l.offsetWidth < mIntParentWidth)
		            l.style.width = mIntParentWidth;

		    }
		    else {//為第三層菜單開始
		        var intLen = document.body.clientWidth - mIntParentWidth - xy[0]
		        if (intLen < l.offsetWidth)//判斷是否到了最右邊
		            s.left = (xy[0] - l.offsetWidth) + "px";
		        else
		            s.left = (xy[0] + mIntParentWidth) + "px"; //控制菜單下拉選項位置
		        s.top = (xy[1]-27) + "px";
		    }			    
		}
...
function stitev(i) {
    ...
    var strOnMouseOver = "";
    if (i.iurl == "") {
        //strOnMouseOut = "funMenuOut(this);";
        strOnMouseOver = "funMenuOver(this);";
    }
	with(i)
	    return ityp == 6 ? "" : " onMouseOver='stitov(event,this,st_ms[" + mei + "].ps[" + ppi + "].is[" + iti + "]);" + strOnMouseOver + "' onMouseOut='stitou(event,this,st_ms[" + mei + "].ps[" + ppi + "].is[" + iti + "]);" + strOnMouseOut + "' onClick='stitck(event,this,st_ms[" + mei + "].ps[" + ppi + "].is[" + iti + "]);'";
}
function stquo(n)
{
	return "\""+n+"\"";
